...
ROOT = Path(__file__).parent          # FusionAL/
MOLPAL_LIB = ROOT / "molpal" / "libraries"
MODEL_ZOO  = ROOT / "models"
OUTPUT_DIR = ROOT / "results" / "embed"

# muben source lives at ROOT/muben/ (symlinked from ALSU)
...
if __name__ == "__main__":
    ...
    _p.add_argument("--dataset",  default=DATASET,
                    choices=["Enamine10k", "Enamine50k", "EnamineHTS"])
    _p.add_argument("--backbone", default="all",
                    choices=["all", "grover", "unimol", "molformer"],
                    help="Which backbone to extract (default: all)")
    _cli = _p.parse_args()
